...
public class groupStudentControl {
    ...
    public ArrayList<groupStudentModel> SelectGroupStudent(int pt_fk_career, int pt_fk_semester, int pt_fk_group, int fk_subject_matter, int pt_fk_period){
        ...
        try {
            ...
            try (Connection conn = new connectionControl().getConexion(); PreparedStatement ps = conn.prepareStatement("CALL `GET_GROUP_BY_STUDENTS`('filterable', "+pt_fk_career+", "+pt_fk_semester+" ,"+pt_fk_group+", "+fk_subject_matter+", "+pt_fk_period+")"); ResultSet res = ps.executeQuery()) {
    ...
    public String DeleteGroupStudent(int dataPkGroupMatterTeacherStudent){
        String request="";
        try {
            try (Connection conn = new connectionControl().getConexion(); PreparedStatement ps = conn.prepareStatement("CALL `SET_GROUP_BY_STUDENT`('delete', "+dataPkGroupMatterTeacherStudent+", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)"); ResultSet res = ps.executeQuery()) {
                while(res!=null&&res.next()){
                    request = res.getString("FL_RESULT");
                }
                res.close();
