            ...
            if(itemCareer!==undefined){
                ...
                if(itemStudyPlan!==undefined){
                    ...
                    if(itemSemester!==undefined){   
                        var filtrableSubjectMatter = {
                            pkCareer : itemCareer.value,
                            pkSemester : itemSemester.value,
                            pkStudyPlan : itemStudyPlan.value,
                            pkPeriod : itemPeriod.value
                        };
                        createDropDownSubjectMatterByCurrentSemester("#qualificationsSubjectMatterFilter", filtrableSubjectMatter, false);
                        ...
                        if(itemSubjectMatter!=null || itemSubjectMatter!=undefined){
                            createDropDownScaleEvaluation("#qualificationsScaleEvaluationFilter",itemLevel.value,false);
                            itemScaleEvaluation = $('#qualificationsScaleEvaluationFilter').jqxDropDownList('getSelectedItem');
                            if(itemScaleEvaluation!=null || itemScaleEvaluation!=undefined){                                
                                existWorkPlanning();
                            }
                        }else{
                            createDropDownScaleEvaluation("#qualificationsScaleEvaluationFilter",null, false);
                            $("#addWorkPlannin").hide();
                            $(".close").parent().hide(); 
                        }    
                    }else{
                ...
                $("#qualificationsSemesterFilter").on('change',function (event){
                    ...
                    if (args) {
                        ...
                        createDropDownSubjectMatterByCurrentSemester("#qualificationsSubjectMatterFilter", filtrableSubjectMatter, true);
                    }
                });
                $("#qualificationsSubjectMatterFilter").on('change',function (event){
                    ...
                    if(args){
                        itemSemester = $('#qualificationsSemesterFilter').jqxDropDownList('getSelectedItem');
                        itemSubjectMatter=$('#qualificationsSubjectMatterFilter').jqxDropDownList('getSelectedItem');
                        createDropDownScaleEvaluation("#qualificationsScaleEvaluationFilter",itemLevel.value, true);
                        itemScaleEvaluation = $('#qualificationsScaleEvaluationFilter').jqxDropDownList('getSelectedItem'); 
            ...
            if(itemScaleEvaluation!==undefined){
                ...
                $.ajax({
                    ...
                    data:{
                        ...
                        "fk_teacher" : itemSubjectMatter.originalItem.dataPkTeacher,
...
//        });
        function loadSource(type){
            if(type==="history"){
                itemPeriod = $('#qualificationsPeriodHistoryFilter').jqxListBox('getSelectedItem');
            }else{
                itemPeriod = $('#qualificationsPeriodFilter').jqxDropDownList('getSelectedItem');
            }
            itemLevel = $('#qualificationsLevelFilter').jqxDropDownList('getSelectedItem');
...
//                        container.append(settingsButton);  
                        ...
                        $('.calendarInput').jqxDateTimeInput({disabled: true, culture: 'es-MX',formatString: "yyyy/MM/dd", theme: theme, width: '120px', height: '30px'});
